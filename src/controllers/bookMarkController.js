...
exports.bookmark = async (req, res) => { // <-- Add 'async' here
  ...
  for (const bookID of uniqueBookIds) {
    ...
    res.render('bookmark', { matchedBooks, bookContainer,latestVolumeUrls, volumes, latestChapterID, chapters  });
  } catch (err) {
...
async function findLatestChapter(latestVolumeUrl) {
  ...
  try {
    ...
    if (chapIDMatch) {
      ...
      console.error('ChapID not found in the latestChapterFileName:', latestChapterFileName);
      return null;
    }
